{{-- resources/views/layouts/sections/menu/vertical-menu.blade.php --}}
{{-- Main vertical sidebar navigation for MOTAC system.
    Filename changed from verticalMenu.blade.php to vertical-menu.blade.php for naming consistency.
    This renders the sidebar with branding and all menu/submenu items.
--}}
    ...
    @if (!($navbarFull ?? false))
        <div class="app-brand demo px-3 py-2 border-bottom">
            <a href="{{ url('/') }}" class="app-brand-link d-flex align-items-center gap-2">
                ...
                <span class="app-brand-text fw-semibold">{{ __($configData['templateName'] ?? 'MOTAC IRMS') }}</span>
            </a>
