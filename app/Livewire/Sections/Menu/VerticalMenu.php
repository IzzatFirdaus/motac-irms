 ...
 * @property-read \Illuminate\Support\Collection|array $userRoles
...
class VerticalMenu extends Component
{
    ...
    public function getFilteredMenuDataProperty()
    {
        ...
        return \collect($menuData)->map(function ($item) use ($userRoles) {
    ...
    private function shouldShowMenuItem($item, $userRoles)
    {
        ...
        if ((isset($item->name) && in_array($item->name, ['menu.reports.title', 'menu.system_settings.title']) || isset($item->menuHeader) && in_array($item->menuHeader, ['menu.section.reports_analytics', 'menu.section.system_settings'])) && (count($userRoles) === 1 && in_array('User', $userRoles))) {
            return false;
        }
