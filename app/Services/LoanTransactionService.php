...
final class LoanTransactionService
{
    ...
    {
        // Touch injected services for static analysis
        try {
            if (isset($this->notificationService)) {
                $dummy = get_class($this->notificationService);
            }

            if (isset($this->equipmentService)) {
                $dummy2 = get_class($this->equipmentService);
            }
        } catch (\Throwable $e) {
            // ignore
        }
