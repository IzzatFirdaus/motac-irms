...
final class LoanTransactionService
{
  ...
  {
    if (empty($itemData)) return LoanTransaction::STATUS_COMPLETED; // Or an error state if no items
    ...
    foreach ($itemData as $item) {
      ...
      if ($status === LoanTransactionItem::STATUS_ITEM_RETURNED_PENDING_INSPECTION) $hasPendingInspection = true;
    ...
    if ($hasPendingInspection) return LoanTransaction::STATUS_RETURNED_PENDING_INSPECTION;
    if ($hasLost && $hasDamaged) return LoanTransaction::STATUS_RETURNED_WITH_DAMAGE_AND_LOSS;
    if ($hasLost) return LoanTransaction::STATUS_RETURNED_WITH_LOSS;
    if ($hasDamaged) return LoanTransaction::STATUS_RETURNED_DAMAGED;
    if ($allGood) return LoanTransaction::STATUS_RETURNED_GOOD; // All items explicitly returned good
