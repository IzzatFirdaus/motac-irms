...
final class LoanApplicationService
{
  ...
  public function createAndSubmitApplication(array $validatedData, User $applicant, bool $saveAsDraftOnly = false): LoanApplication
  {
      ...
      // --- EDITED CODE: START ---
      // Changed the array key from 'items' to 'loan_application_items' to match the component.
      if (empty($validatedData['loan_application_items'])) {
      ...
      return DB::transaction(function () use ($validatedData, $applicant, $applicantId, $saveAsDraftOnly) {
          ...
          // --- EDITED CODE: START ---
          // Changed the array key from 'items' to 'loan_application_items' here as well.
          if (!empty($validatedData['loan_application_items'])) {
              foreach ($validatedData['loan_application_items'] as $item) {
  ...
  public function updateApplication(LoanApplication $application, array $validatedData, User $user): LoanApplication
  {
    ...
    return DB::transaction(function () use ($application, $validatedData, $user) {
        ...
        // --- EDITED CODE: START ---
        // Changed the array key from 'items' to 'loan_application_items'.
        if (isset($validatedData['loan_application_items']) && is_array($validatedData['loan_application_items'])) {
            $this->syncApplicationItems($application, $validatedData['loan_application_items']);
        }
        // --- EDITED CODE: END ---
