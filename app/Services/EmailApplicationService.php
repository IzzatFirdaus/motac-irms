...
final class EmailApplicationService
{
    ...
    private function submitApplicationLogic(EmailApplication $application, User $submitter): EmailApplication
    {
        ...
        if ($application->supporting_officer_id) {
            ...
            if (!$supportingOfficerModel) throw new ModelNotFoundException(__('Pegawai Penyokong (dari sistem) yang dipilih tidak ditemui.'));
        ...
        if ($approvalTask && $supportingOfficerModel) $this->notificationService->notifyApproverApplicationNeedsAction($approvalTask, $application, $supportingOfficerModel);
        elseif ($manualOfficerDetails) Log::info(self::LOG_AREA . "Approval for Email App ID: {$application->id} relies on manual officer: {$manualOfficerDetails['officer_email']}. Manual notification may be required.");
    ...
    public function processProvisioning(EmailApplication $application, array $provisioningDetails, User $actingAdmin): EmailApplication
    {
        ...
        return DB::transaction(function () use ($application, $provisioningDetails, $actingAdmin) {
            ...
            if (empty($targetEmail) && empty($targetUserId)) throw new InvalidArgumentException(__('E-mel rasmi atau ID Pengguna yang akan disediakan adalah mandatori.'));
            ...
            if ($provisionResult['success']) {
                ...
                if ($application->user) {
                    ...
                    if ($application->final_assigned_email) $userToUpdate->motac_email = $application->final_assigned_email;
                    if ($application->final_assigned_user_id) $userToUpdate->user_id_assigned = $application->final_assigned_user_id;
                    $userToUpdate->save();
                }
                Log::info(self::LOG_AREA."Provisioning COMPLETED.", ['id' => $application->id, 'assigned_email' => $application->final_assigned_email]);
                if ($application->user && method_exists($this->notificationService, 'notifyApplicantEmailProvisioned')) $this->notificationService->notifyApplicantEmailProvisioned($application);
            } else {
                ...
                if (method_exists($this->notificationService, 'notifyAdminProvisioningFailed')) {
                    ...
                    if ($itAdmins->isNotEmpty()) $this->notificationService->notifyAdminProvisioningFailed($application, $failureReason, $itAdmins, $actingAdmin);
                    else Log::warning(self::LOG_AREA . "No IT Admins found to notify for provisioning failure of App ID: " . $application->id);
                }
            }
            $application->save();
            return $application->fresh($this->defaultEmailApplicationRelations);
    ...
    public function deleteApplication(EmailApplication $application, User $deleter): bool
    {
        ...
        try {
            return DB::transaction(function () use ($application) {
                ...
                if ($result) Log::info(self::LOG_AREA . "Soft DELETED successfully.", ['id' => $application->id]);
                else Log::warning(self::LOG_AREA . "Soft delete operation returned false.", ['id' => $application->id]);
                return (bool)$result;
            });
        } catch (Throwable $e) {
