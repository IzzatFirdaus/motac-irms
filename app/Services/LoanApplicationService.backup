...
final class LoanApplicationService
{
  ...
  public function createAndSubmitApplication(array $validatedData, User $applicant, bool $saveAsDraftOnly = false): LoanApplication
  {
      ...
      return DB::transaction(function () use ($validatedData, $applicant, $applicantId, $saveAsDraftOnly) { //
          $applicationModelData = [ //
              ...
              // *** MODIFICATION: Include supporting_officer_id ***
          ...
          if (!empty($validatedData['items'])) { //
              // Ensure items are only active ones if already filtered by ApplicationForm.php
              // This service method assumes $validatedData['items'] are the ones to be created.
  ...
  public function submitApplicationForApproval(LoanApplication $application, User $submitter): LoanApplication
  {
    ...
    if($application->supporting_officer_id){ // Check if supporting_officer_id is set on the application
        ...
        Log::warning(self::LOG_AREA . "Submitting application without a supporting officer assigned.", ['application_id' => $application->id]); //
    }
    ...
    return DB::transaction(function () use ($application, $submitter, $supportingOfficer) { //
      ...
      $approvalTask = null; //
      $assignedOfficerForNextStage = null;

      if ($supportingOfficer) { //
          $assignedOfficerForNextStage = $supportingOfficer;
          $approvalTask = $this->approvalService->initiateApprovalWorkflow( //
              $application, //
              $submitter, //
              Approval::STAGE_LOAN_SUPPORT_REVIEW, //
              $assignedOfficerForNextStage //
          );
      } else { //
          Log::info(self::LOG_AREA . "No supporting officer; attempting to find officer for Approver Review stage.", ['application_id' => $application->id]); //
          $assignedOfficerForNextStage = $this->approvalService->findOfficerForStage($application, Approval::STAGE_LOAN_APPROVER_REVIEW, $submitter); //
          if($assignedOfficerForNextStage) { //
            $approvalTask = $this->approvalService->initiateApprovalWorkflow( //
                $application, //
                $submitter, //
                Approval::STAGE_LOAN_APPROVER_REVIEW, //
                $assignedOfficerForNextStage //
            );
          } else {
             Log::error(self::LOG_AREA . "No officer found for Approver Review stage. Application ID: {$application->id} remains in {$application->status} but may not have an active approval task."); //
             $adminEmail = config('motac.notifications.admin_email_recipient'); //
             if ($adminEmail) {
                 $title = __('Tindakan Diperlukan: Permohonan Pinjaman ICT Memerlukan Penetapan Pelulus');
                 $message = __('Permohonan Pinjaman ICT ID #:application_id tidak dapat diberikan kepada pegawai pelulus secara automatik. Sila semak dan tetapkan pegawai pelulus secara manual.', ['application_id' => $application->id]);

                 $adminUser = User::role('Admin')->where('email', $adminEmail)->first(); //
                 if (!$adminUser) {
                    $adminUser = User::role('Admin')->first(); //
                 }

                 if ($adminUser) {
                    $this->notificationService->notifyUserWithDefaultNotification( //
                        $adminUser, //
                        $title,
                        $message,
                        $application, //
                        route('loan-applications.show', $application->id) //
                    );
                     Log::info(self::LOG_AREA."Orphaned application notification dispatched to admin: {$adminUser->email} for LoanApplication ID: {$application->id}.");
                 } else {
                     Log::error(self::LOG_AREA."No admin user with 'Admin' role found to notify for orphaned LoanApplication ID: {$application->id}. Configured admin email was {$adminEmail}.");
                 }
             }
          }
      }
  ...
  public function updateApplication(LoanApplication $application, array $validatedData, User $user): LoanApplication
  {
    ...
    // This validation for supporting_officer_id should ideally be in a FormRequest or handled by the calling component (ApplicationForm.php)
    // However, keeping it here as a safeguard if data comes from other sources or if component validation is bypassed.
    ...
    return DB::transaction(function () use ($application, $validatedData, $user) { //
        // Use Arr::only to ensure only fillable fields are taken from $validatedData.
        // This now includes supporting_officer_id if it's in $validatedData and fillable.
        $applicationModelData = Arr::only($validatedData, $application->getFillable()); //

        // Explicitly handle responsible_officer_id based on applicant_is_responsible_officer flag
        // This overrides what Arr::only might have picked up if responsible_officer_id was also in $validatedData.
        if (isset($validatedData['applicant_is_responsible_officer'])) { //
            ...
            } elseif (isset($validatedData['responsible_officer_id'])) { // If flag is false and ID is provided
                $applicationModelData['responsible_officer_id'] = $validatedData['responsible_officer_id']; //
            } else { // If flag is false and ID is NOT provided (e.g. draft where it wasn't selected yet)
                 // This might be set to null if not requiredIf for draft in ApplicationForm validation.
                 // Arr::only would have already set it if present in $validatedData and fillable.
                 // If it was not in $validatedData, and applicant_is_responsible_officer is false,
                 // then it should be whatever was in Arr::only (possibly null or existing value if not in validatedData)
                 // or explicitly set to null if that's the desired logic.
                 // The current logic with Arr::only and then conditional override for responsible_officer_id seems fine.
                 // $applicationModelData['responsible_officer_id'] = $validatedData['responsible_officer_id'] ?? null;
            }
        }
        // supporting_officer_id is handled by Arr::only as it's directly in $validatedData from ApplicationForm.php

        $application->fill($applicationModelData); //

        if (array_key_exists('applicant_confirmation', $validatedData)) { //
            // Determine if this update is effectively for a draft save, even if not explicitly passed
            ...
            if (!$isStillDraft && ($validatedData['applicant_confirmation'] ?? false) === true) { //
                 $application->applicant_confirmation_timestamp = $application->applicant_confirmation_timestamp ?? now(); // Set only if not already set (for resubmissions)
            } elseif (($validatedData['applicant_confirmation'] ?? null) === false || $isStillDraft) { // If unconfirmed or saving as draft
                 $application->applicant_confirmation_timestamp = null; // Clear it
            }
        }
        ...
        if (isset($validatedData['items']) && is_array($validatedData['items'])) { //
            // $validatedData['items'] from ApplicationForm.php now contains only active items.
  ...
  protected function syncApplicationItems(LoanApplication $application, array $itemsData): void
  {
    ...
    foreach ($itemsData as $itemData) { //
      // This method receives $itemsData which are already filtered active items from ApplicationForm.php
      ...
      if ($itemId && in_array($itemId, $existingItemIds, true)) { //
        ...
        // No _delete check needed here as $itemsData are active items
        if ($quantity > 0) { //
            LoanApplicationItem::find($itemId)?->update($itemPayload); //
            $processedItemIds[] = $itemId; //
        } else {
            // If quantity became 0 for an existing item, it should be deleted.
            // This is handled by it not being in $processedItemIds and thus part of $idsToDelete.
      ...
      } elseif ($quantity > 0) { // New item, (no _delete check needed here)
