...
class ReportController extends Controller
{
    ...
    public function loanHistory(Request $request): View
    {
        ...
        if ($request->filled('user_id')) {
            $query->whereHas('loanApplication', fn ($q) => $q->where('user_id', $request->input('user_id')));
        }
        if ($request->filled('equipment_id')) {
            $query->whereHas('loanTransactionItems', fn ($q) => $q->where('equipment_id', $request->input('equipment_id')));
        }
