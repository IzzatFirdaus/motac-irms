...
This document lists all specified colours, organized by category, usage, and mode (Light/Dark). Each
entry includes the name, variable, and associated shade/code.

This system uses two layers of tokens:

1. **Primitive Tokens**: Foundational color values (e.g., `primary-600`, `#2563EB`). These are the
   raw color swatches.
2. **Semantic Tokens**: Tokens that describe a color's purpose (e.g., `bg-primary-600`,
   `txt-primary`). These semantic tokens map to different primitive tokens depending on the active
   theme (Light or Dark).

_Citations: Token structure and mapping confirmed by reference images and design system visual
guides._
...
This table shows how the primitive gray tokens are re-mapped in dark mode. For instance, requesting
`gray-100` in dark mode will return the hex code for `gray-900`.
...
_Citations: Success semantic background tokens confirmed by bg-token-6.png and bg-token-6-dark.png
images._
...
_Citations: Warning semantic background tokens confirmed by bg-token-7.png and bg-token-7-dark.png
images. Typo corrected to use bg-warning-400._
