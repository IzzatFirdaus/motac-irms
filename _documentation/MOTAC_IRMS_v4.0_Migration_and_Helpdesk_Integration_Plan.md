...
This document outlines the comprehensive migration plan for refactoring MOTAC IRMS from a
dual-purpose (Email/User ID Provisioning & ICT Equipment Loan) system to a focused ICT Equipment
Loan and Helpdesk ticketing system.

---

## Objectives

- **Remove Email/User ID Provisioning**: Eliminate all code, database tables, and UI elements
  related to the Email/User ID Provisioning module.
- **Integrate Helpdesk System**: Add a new module for IT support ticket management.
- **Maintain ICT Equipment Loan**: Ensure the ICT Equipment Loan Management system remains robust
  and functional.
...
- **Notifications**:
  - Email-application-specific classes (e.g., `ApplicationSubmitted.php`,
    `EmailApplicationNeedsAction.php`)
- **Config & Language**:
  - Remove email-related keys in `config/motac.php`
  - Remove translation strings under "email" or "email_application" in `resources/lang/en/` and
    `resources/lang/ms/`
- **Tests**:
...
- Remove or adjust logic in `User.php`, `ReportController.php`, `NotificationService.php`, and
  service providers corresponding to EmailApplication, where applicable.
...
- **tickets**
  - id, user_id, assigned_to_user_id, category_id, priority_id, subject, description, status (enum),
    due_date, resolution_notes, closed_at, created_by, updated_by, deleted_by, timestamps,
    deleted_at
- **ticket_categories**
...
- **ticket_comments**
  - id, ticket_id, user_id, comment, is_internal, created_by, updated_by, deleted_by, timestamps,
    deleted_at
- **ticket_attachments** (polymorphic)
  - id, attachable_type, attachable_id, file_path, file_name, file_size, file_type, created_by,
...
- Update `config/motac.php` to add Helpdesk-related keys (e.g., default assignment rules, allowed
  attachment types, helpdesk email).
...
**For team circulation: If you need this plan in PDF format for distribution, export this Markdown
document using your preferred tool.**
