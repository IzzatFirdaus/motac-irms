...
$(function () {
    ...
    dt_user = dt_user_table.DataTable({
      ...
      ajax: {
        ...
        error: function error(xhr, _error2, thrown) {
          console.error('DataTables AJAX Error:', _error2);
        }
      },
      ...
      columnDefs: [{
        ...
        targets: 1,
        render: function render(data, type, full, meta) {
          return "<span class=\"fw-medium\">".concat(full.fake_id || full.id, "</span>");
        }
      }, {
        /**
        ...
        render: function render(data, type, full, meta) {
          ...
          var $row_output = '<div class="d-flex justify-content-start align-items-center user-name">' + '<div class="avatar-wrapper">' + '<div class="avatar avatar-sm me-3">' + $avatarOutput + '</div>' + '</div>' + '<div class="d-flex flex-column">' + '<a href="' + userView + '" class="text-body text-truncate text-decoration-none">' + '<span class="fw-semibold">' + $name + '</span>' + '</a>' +
        ...
        render: function render(data, type, full, meta) {
          ...
          if ($verified) {
            return '<span class="badge bg-success-subtle text-success border border-success-subtle rounded-pill">' + '<i class="ti ti-shield-check me-1"></i>Verified' + '</span>';
          } else {
            return '<span class="badge bg-danger-subtle text-danger border border-danger-subtle rounded-pill">' + '<i class="ti ti-shield-x me-1"></i>Unverified' + '</span>';
          }
        }
      }, {
        /**
        ...
        render: function render(data, type, full, meta) {
          ...
          '<button class="btn btn-sm btn-icon btn-text-secondary rounded-pill waves-effect edit-record" ' + 'data-id="' + full['id'] + '" ' + 'data-bs-toggle="offcanvas" ' + 'data-bs-target="#offcanvasAddUser" ' + 'data-bs-original-title="Edit User" ' + 'title="Edit User">' + '<i class="ti ti-edit ti-md"></i>' + '</button>' +
          // Delete button - shows confirmation dialog
          '<button class="btn btn-sm btn-icon btn-text-secondary rounded-pill waves-effect delete-record" ' + 'data-id="' + full['id'] + '" ' + 'data-bs-original-title="Delete User" ' + 'title="Delete User">' + '<i class="ti ti-trash ti-md"></i>' + '</button>' +
          // Dropdown menu for additional actions
          '<div class="dropdown">' + '<button class="btn btn-sm btn-icon btn-text-secondary rounded-pill waves-effect dropdown-toggle hide-arrow" ' + 'data-bs-toggle="dropdown" aria-expanded="false">' + '<i class="ti ti-dots-vertical ti-md"></i>' + '</button>' + '<ul class="dropdown-menu dropdown-menu-end">' + '<li><a class="dropdown-item" href="' + userView + '"><i class="ti ti-eye me-2"></i>View Details</a></li>' + '<li><a class="dropdown-item" href="javascript:void(0);"><i class="ti ti-user-pause me-2"></i>Suspend User</a></li>' + '<li><a class="dropdown-item" href="javascript:void(0);"><i class="ti ti-lock me-2"></i>Reset Password</a></li>' + '<li><hr class="dropdown-divider"></li>' + '<li><a class="dropdown-item text-danger" href="javascript:void(0);"><i class="ti ti-user-x me-2"></i>Deactivate</a></li>' + '</ul>' + '</div>' + '</div>';
        }
      }],
      // Default ordering - sort by name column in descending order
      order: [[2, 'desc']],
      // Custom DOM structure for better layout control
      dom: '<"row mx-1"' + '<"col-sm-12 col-md-3"' + '<"me-3"l>' +
      // Length selector
      '>' + '<"col-sm-12 col-md-9"' + '<"dt-action-buttons text-xl-end text-lg-start text-md-end text-start d-flex align-items-center justify-content-end flex-md-row flex-column mb-3 mb-md-0 gap-3"fB>' +
      // Filter and Buttons
      '>' + '>t' +
      // Table
      '<"row mx-2"' + '<"col-sm-12 col-md-6"i>' +
      // Info
      ...
      buttons: [{
        ...
        buttons: [{
          ...
          customize: function customize(win) {
            ...
            $(win.document.body).css('color', '#333').css('border-color', '#ddd').css('background-color', '#fff');
            $(win.document.body).find('table').addClass('compact').css('color', 'inherit').css('border-color', 'inherit').css('background-color', 'inherit');

            // Add custom header
            $(win.document.body).prepend('<div style="text-align: center; margin-bottom: 20px;">' + '<h2>MOTAC Integrated Resource Management System</h2>' + '<h3>Users Report</h3>' + '<p>Generated on: ' + new Date().toLocaleDateString() + '</p>' + '</div>');
          }
        }, {
          extend: 'csv',
        ...
        details: {
          ...
          renderer: function renderer(api, rowIdx, columns) {
            var data = $.map(columns, function (col, i) {
              return col.title !== '' ? '<tr data-dt-row="' + col.rowIndex + '" data-dt-column="' + col.columnIndex + '">' + '<td class="fw-medium">' + col.title + ':</td> ' + '<td>' + col.data + '</td>' + '</tr>' : '';
            }).join('');
  ...
  $(document).on('click', '.edit-record', function () {
    ...
    $.get("".concat(baseUrl, "user-list/").concat(user_id, "/edit")).done(function (data) {
   ...
   */
  if (addNewUserForm) {
    var _fv = FormValidation.formValidation(addNewUserForm, {
      fields: {
        name: {
          validators: {
            notEmpty: {
      ...
      var url = isEdit ? "".concat(baseUrl, "user-list/").concat($('#user_id').val()) : "".concat(baseUrl, "user-list");
      ...
      $.ajax({
        ...
        success: function success(response) {
          ...
          Swal.fire({
            icon: 'success',
            title: "User ".concat(isEdit ? 'Updated' : 'Created', " Successfully!"),
            text: "The user has been ".concat(isEdit ? 'updated' : 'created', " successfully."),
            customClass: {
              confirmButton: 'btn btn-success waves-effect waves-light'
            },
            buttonsStyling: false
        ...
        error: function error(xhr, status, _error4) {
          ...
          // Show error message
          Swal.fire({
            title: "".concat(isEdit ? 'Update' : 'Creation', " Failed"),
            text: errorMessage,
            icon: 'error',
            customClass: {
              confirmButton: 'btn btn-primary waves-effect waves-light'
  ...
  dt_user_table.on('draw.dt', function () {
    var info = dt_user.page.info();
    var announcement = "Table updated. Showing ".concat(info.start + 1, " to ").concat(info.end, " of ").concat(info.recordsTotal, " users.");

    // Create or update live region for announcements
    var liveRegion = $('#table-announcements');
