...
<div style="text-align: center; padding-bottom: 15px; border-bottom: 1px solid #e0e0e0; margin-bottom: 20px;">
    {{-- Ensure this path is absolute or correctly resolved in email context --}}
    ...
    <h1 style="font-size: 20px; color: #0050A0; margin:0;">{{ $appName ?? config('app.name', __('Sistem Pengurusan Sumber MOTAC')) }}</h1>
</div>
...
@component('mail::button', ['url' => $actionUrl, 'color' => $actionColor ?? 'motac_primary']) {{-- 'motac_primary' or other color to be defined in theme --}}
{{ __($actionText) }}
@endcomponent
@endif
...
{{-- Subcopy (Footer Note) --}}
@isset($subcopy) {{-- Allow custom subcopy --}}
    @slot('subcopy')
        {{ $subcopy }}
    @endslot
@else
