...
<head>
    ...
    <style>
        ...
        .alert-secondary p { margin-bottom: 0.5rem; }
        .alert-secondary strong { display: inline-block; min-width: 150px; }
...
<body>
    <div class="email-container">
                    ...
                    <tbody>
                        @foreach ($loanTransaction->loanTransactionItems as $item)
                            <tr>
                                ...
                                <td>{{ $item->status_on_return_translated ?? ($loanTransaction->status_translated ?? __('N/A')) }}</td>
                                <td>{{ $loanTransaction->transaction_date?->translatedFormat(config('app.datetime_format_my', 'd/m/Y H:i A')) ?? 'N/A' }}</td>
        ...
        <p>{{ __('Jika anda mempunyai sebarang pertanyaan mengenai pulangan peralatan ini, sila hubungi Bahagian Pengurusan Maklumat (BPM).') }}</p>
