...
<head>
    ...
    <style>
        ...
        .alert-warning p { margin-bottom: 0.5rem; }
        .alert-warning strong { display: inline-block; min-width: 150px; }
...
<body>
    <div class="email-container">
        ...
        <p>{{ __('Rekod kami menunjukkan bahawa peralatan yang dipinjam di bawah permohonan ini telah') }} <strong class="text-danger">{{ __('LEWAT DIPULANGKAN') }}</strong>.</p>
        <div class="alert-details alert-warning">
            ...
            <p><strong>{{ __('Tarikh Dijangka Pulang') }}:</strong> <span class="text-danger">{{ $loanApplication->loan_end_date?->translatedFormat(config('app.date_format_my', 'd/m/Y')) ?? 'N/A' }}</span></p>
                    ...
                    <tbody>
                        @foreach ($overdueItems as $item)
                            <tr>
                                ...
                                <td>{{ $item->transaction->transaction_date?->translatedFormat(config('app.datetime_format_my', 'd/m/Y H:i A')) ?? 'N/A' }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            @else
                <p>{{ __('Tiada butiran spesifik peralatan yang lewat tempoh dikenal pasti berdasarkan rekod terkini atau kesemua item telah direkodkan sebagai dipulangkan.') }}</p>
            @endif
            <p style="margin-top: 1rem; margin-bottom:0;">
                <strong>{{ __('Sila pulangkan peralatan tersebut ke Bahagian Pengurusan Maklumat (BPM) dengan kadar segera.') }}</strong>
            </p>
        </div>
        <p>{{ __('Jika anda telah memulangkan peralatan tersebut baru-baru ini, sila abaikan e-mel ini atau hubungi kami untuk pengesahan.') }}</p>
        <p>{{ __('Kegagalan memulangkan peralatan mengikut tempoh yang ditetapkan boleh menyebabkan tindakan selanjutnya diambil.') }}</p>
