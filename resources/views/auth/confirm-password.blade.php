        ...
        <div class="authentication-inner row m-0">
            ...
            <div class="d-flex col-12 col-lg-5 align-items-center authentication-bg p-sm-5 p-4">
                <div class="w-px-400 mx-auto">
                    ...
                    <p class="text-center mb-4 text-muted small">{{ __('Ini adalah kawasan selamat aplikasi. Sila sahkan kata laluan anda sebelum meneruskan.') }}</p> {{-- Clarified text based on Jetstream's default --}}

                    <x-validation-errors class="mb-3" /> {{-- Ensure this component renders errors correctly --}}
                    ...
                    <form id="formConfirmPassword" method="POST" action="{{ route('password.confirm') }}">
                        ...
                        <div class="mb-3 form-password-toggle">
                            ...
                            @error('password')
                                <span class="invalid-feedback d-block" role="alert"> {{-- Ensure d-block if not already default --}}
                                    <span class="fw-medium">{{ $message }}</span>
                                </span>
                            @enderror
                        </div>
                        <button type="submit" class="btn btn-primary d-grid w-100 mb-3">
                            <i class="bi bi-check-circle-fill me-1"></i>{{ __('Sahkan') }} {{-- Changed from Sahkan Kata Laluan to just Sahkan for brevity --}}
                        </button>
                    </form>
                </div>
            </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.toggle-password').forEach(function(toggle) {
                toggle.addEventListener('click', function() {
                    const input = this.closest('.input-group').querySelector('input'); // More robust input selection
