    ...
    document.addEventListener('DOMContentLoaded', function() {
        if (typeof toastr !== 'undefined') {
            ...
            window.addEventListener('toastr', event => {
                if (event.detail && typeof event.detail.type === 'string' && typeof event.detail
                    .message === 'string') {
                    toastr[event.detail.type](event.detail.message, event.detail.title ?? '');
                } else {
                    console.warn('Toastr event invalid:', event.detail);
                }
            });
        } else {
        ...
        const getBsInstance = (id, Comp) => {
            ...
            return el ? Comp.getInstance(el) || new Comp(el) : null;
        };
        ...
        window.addEventListener('openCanvas', e => getBsInstance(e.detail?.elementId, bootstrap.Offcanvas)
            ?.show());
        window.addEventListener('closeCanvas', e => getBsInstance(e.detail?.elementId, bootstrap.Offcanvas)
            ?.hide());
        window.addEventListener('scrollToTop', () => window.scrollTo({
            top: 0,
            behavior: 'smooth'
        }));
