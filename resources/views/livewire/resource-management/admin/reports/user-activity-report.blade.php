        ...
        <div class="card-body p-3 p-md-4">
            ...
            <div class="row g-3 align-items-end">
                <div class="col-lg-4 col-md-6">
                    <label for="repUsrActSearch" class="form-label small">{{ __('Carian (Nama Pengguna, Emel)') }}</label>
                ...
                <thead class="table-light">
                    <tr>
                        <th class="small text-uppercase text-muted fw-medium px-3 py-2" wire:click="setSortBy('name')"
                            ...
                            {{ __('Nama Pengguna') }}
                            @if ($sortBy === 'name')
                                <i class="bi {{ $sortDirection === 'asc' ? 'bi-sort-alpha-down' : 'bi-sort-alpha-up-alt' }} ms-1"></i>
                            @else
                        ...
                        <th class="small text-uppercase text-muted fw-medium px-3 py-2 text-center"
                            ...
                            @if ($sortBy === 'email_applications_count')
                                <i class="bi {{ $sortDirection === 'asc' ? 'bi-sort-numeric-down' : 'bi-sort-numeric-up-alt' }} ms-1"></i>
                            @else
                                <i class="bi bi-arrow-down-up text-muted ms-1" style="font-size: 0.8em;"></i>
                            @endif
                        </th>
                        ...
                        <tr wire:key="user-act-report-{{ $item->id }}">
                            ...
                            <td class="px-3 py-2 small text-muted">
                                @forelse($item->roles as $role)
                                    <span class="badge bg-secondary-subtle text-secondary-emphasis fw-normal me-1">{{ $role->name }}</span>
                                @empty
