<div>
    <div class="card shadow-sm mb-4">
        <div class="card-header bg-light py-3">
            <div class="d-flex flex-wrap align-items-center justify-content-between">
                <h3 class="h5 mb-0 fw-semibold text-dark">
                    <i class="bi bi-envelope-paper-fill me-2"></i>{{ __('Laporan Akaun E-mel & ID Pengguna') }}
                </h3>
                @if (Route::has('reports.index')) {{-- Assuming a general reports index page --}}
        ...
        <div class="card-body p-3 p-md-4">
            <x-alert-bootstrap /> {{-- For session messages if any --}}

            {{-- Filters --}}
            <div class="row g-3 mb-4 align-items-end">
                <div class="col-md-3">
                    <label for="searchTerm" class="form-label form-label-sm">{{ __('Carian Nama/Emel') }}</label>
                    <input wire:model.live.debounce.300ms="searchTerm" type="search" class="form-control form-control-sm" id="searchTerm" placeholder="{{ __('Cth: nama, emel@motac...') }}">
                </div>
                <div class="col-md-2">
                    <label for="filterStatus" class="form-label form-label-sm">{{ __('Status Permohonan') }}</label>
                    <select wire:model.live="filterStatus" class="form-select form-select-sm" id="filterStatus">
                    ...
                    <table class="table table-hover table-striped table-sm mb-0">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th class="small" wire:click="setSortBy('proposed_email')" style="cursor: pointer;">{{ __('Emel Dicadang') }} <x-sort-icon field="proposed_email" :sortField="$sortBy" :sortDirection="$sortDirection" /></th>
                                <th class="small" wire:click="setSortBy('final_assigned_email')" style="cursor: pointer;">{{ __('Emel Dilulus') }} <x-sort-icon field="final_assigned_email" :sortField="$sortBy" :sortDirection="$sortDirection" /></th>
                                ...
                                <th class="small" wire:click="setSortBy('updated_at')" style="cursor: pointer;">{{ __('Tarikh Kemaskini') }} <x-sort-icon field="updated_at" :sortField="$sortBy" :sortDirection="$sortDirection" /></th>
                            </tr>
                        </thead>
                        <tbody>
