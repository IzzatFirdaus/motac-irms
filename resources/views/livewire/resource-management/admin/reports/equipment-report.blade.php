{{-- resources/views/livewire/reports/equipment-report.blade.php --}}
<div>
        ...
        <div class="card-header bg-light py-3 motac-card-header"> {{-- Ensure bg-light uses MOTAC theme surface color --}}
            <div class="d-flex flex-wrap align-items-center justify-content-between">
                <h3 class="h5 mb-0 fw-semibold text-dark d-flex align-items-center">
                    <i class="bi bi-laptop-fill me-2"></i>{{-- Already Bootstrap Icon --}}
            ...
            <div class="row g-3 mb-4 align-items-end">
                <div class="col-md-3 col-lg-2">
                    ...
                    <input wire:model.live.debounce.300ms="searchTerm" type="search" id="searchTermEquip"
                        class="form-control form-control-sm" placeholder="{{ __('Cth: MOTAC/LPT/001') }}"> {{-- Ensure form-control is MOTAC themed --}}
                </div>
                <div class="col-md-2 col-lg-2">
                    <label for="filterAssetType" class="form-label form-label-sm">{{ __('Jenis Aset') }}</label>
                    <select wire:model.live="filterAssetType" id="filterAssetType" class="form-select form-select-sm"> {{-- Ensure form-select is MOTAC themed --}}
                ...
                <div class="col-md-1 col-lg-1">
                    <button wire:click="resetFilters" class="btn btn-sm btn-outline-secondary w-100"
                        type="button">{{ __('Reset') }}</button> {{-- Ensure btn-outline-secondary is MOTAC themed --}}
                </div>
            </div>
                    ...
                    <table class="table table-hover table-striped table-sm mb-0"> {{-- Ensure table is MOTAC themed --}}
                        <thead class="table-light"> {{-- Ensure table-light uses MOTAC theme header bg --}}
                            <tr>
                                ...
                                <th class="small text-uppercase text-muted fw-medium px-3 py-2" wire:click="setSortBy('tag_id')" style="cursor: pointer;">
                                    {{ __('Tag ID') }} <x-sort-icon field="tag_id" :sortField="$sortBy"
                                        :sortDirection="$sortDirection" /></th>
                                <th class="small text-uppercase text-muted fw-medium px-3 py-2" wire:click="setSortBy('item_code')" style="cursor: pointer;">
                                    {{ __('Kod Item') }} <x-sort-icon field="item_code" :sortField="$sortBy"
                                        :sortDirection="$sortDirection" /></th>
                                <th class="small text-uppercase text-muted fw-medium px-3 py-2" wire:click="setSortBy('asset_type')" style="cursor: pointer;">
                                    {{ __('Jenis Aset') }} <x-sort-icon field="asset_type" :sortField="$sortBy"
                                        :sortDirection="$sortDirection" /></th>
                                <th class="small text-uppercase text-muted fw-medium px-3 py-2" wire:click="setSortBy('brand')" style="cursor: pointer;">
                                    {{ __('Jenama') }} <x-sort-icon field="brand" :sortField="$sortBy"
                                        :sortDirection="$sortDirection" /></th>
                                <th class="small text-uppercase text-muted fw-medium px-3 py-2" wire:click="setSortBy('model')" style="cursor: pointer;">
                                    {{ __('Model') }} <x-sort-icon field="model" :sortField="$sortBy"
                                        :sortDirection="$sortDirection" /></th>
                                <th class="small text-uppercase text-muted fw-medium px-3 py-2" wire:click="setSortBy('serial_number')" style="cursor: pointer;">
                                    {{ __('No. Siri') }} <x-sort-icon field="serial_number" :sortField="$sortBy"
                                        :sortDirection="$sortDirection" /></th>
                                <th class="small text-uppercase text-muted fw-medium px-3 py-2" wire:click="setSortBy('status')" style="cursor: pointer;">
                                    {{ __('Status Operasi') }} <x-sort-icon field="status" :sortField="$sortBy"
                                        :sortDirection="$sortDirection" /></th>
                                <th class="small text-uppercase text-muted fw-medium px-3 py-2" wire:click="setSortBy('condition_status')"
                                    style="cursor: pointer;">{{ __('Status Keadaan') }} <x-sort-icon
                                        field="condition_status" :sortField="$sortBy" :sortDirection="$sortDirection" /></th>
                                <th class="small text-uppercase text-muted fw-medium px-3 py-2" wire:click="setSortBy('locations.name')" style="cursor: pointer;">
                                    {{ __('Lokasi') }} <x-sort-icon field="locations.name" :sortField="$sortBy"
                                        :sortDirection="$sortDirection" /></th>
                                <th class="small text-uppercase text-muted fw-medium px-3 py-2" wire:click="setSortBy('departments.name')"
