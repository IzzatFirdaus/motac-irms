{{-- resources/views/loan-applications/create.blade.php --}}
@extends('layouts.app')
    ...
    <div class="row justify-content-center">
        <div class="col-lg-10 col-xl-9">
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 pb-2 border-bottom">
                <h1 class="h2 fw-bold text-dark mb-0">{{ __('Borang Permohonan Pinjaman Peralatan ICT') }}</h1>
            ...
            {{-- Removed @include('partials.validation-errors-alt') --}}
            {{-- Removed @include('partials.alert-messages') --}}
            {{-- Assuming global alert handling (e.g., _partials._alerts.alert-general.blade.php) is in layouts.app --}}
            ...
            <form action="{{ route('loan-applications.store') }}" method="POST" id="loanApplicationCreateForm">
                    ...
                    <div class="card-body p-4">
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <p class="form-control-plaintext bg-light px-3 py-2 rounded-3 border">{{ Auth::user()->name ?? __('N/A') }}</p>
                            </div>
                            <div class="col-md-6">
                                <label for="applicant_phone" class="form-label fw-semibold">{{ __('No. Telefon (Untuk Dihubungi)') }}<span class="text-danger">*</span></label>
                                <input type="text" name="applicant_phone" id="applicant_phone" class="form-control @error('applicant_phone') is-invalid @enderror" value="{{ old('applicant_phone', optional(Auth::user())->mobile_number ?? (optional(Auth::user())->phone_number ?? '')) }}" placeholder="Cth: 012-3456789" required>
                            ...
                            <div class="col-md-6">
                                ...
                                <p class="form-control-plaintext bg-light px-3 py-2 rounded-3 border">{{ e(optional(optional(Auth::user())->position)->name) ?? __('N/A') }} ({{ e(optional(optional(Auth::user())->grade)->name) ?? __('N/A') }})</p>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-medium small text-muted">{{ __('Bahagian/Unit:') }}</label>
                                <p class="form-control-plaintext bg-light px-3 py-2 rounded-3 border">{{ e(optional(optional(Auth::user())->department)->name) ?? __('N/A') }}</p>
                            </div>
                        </div>
                        <hr class="my-4">
                        <div class="mb-3">
                            <label for="purpose" class="form-label fw-semibold">{{ __('Tujuan Permohonan') }}<span class="text-danger">*</span></label>
                            <textarea name="purpose" id="purpose" class="form-control @error('purpose') is-invalid @enderror" rows="3" required placeholder="Nyatakan tujuan permohonan peralatan ICT secara terperinci...">{{ old('purpose') }}</textarea>
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="location" id="location" class="form-control @error('location') is-invalid @enderror" required value="{{ old('location') }}" placeholder="Cth: Bilik Mesyuarat Utama, Aras 10, MOTAC">
                                @error('location') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>
                             <div class="col-md-6">
                                <label for="return_location" class="form-label fw-semibold">{{ __('Lokasi Dijangka Pulang / Pemulangan') }}</label>
                                <input type="text" name="return_location" id="return_location" class="form-control @error('return_location') is-invalid @enderror" value="{{ old('return_location') }}" placeholder="Cth: Kaunter BPM (Jika berbeza dari lokasi guna)">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="datetime-local" name="loan_start_date" id="loan_start_date" class="form-control @error('loan_start_date') is-invalid @enderror" required value="{{ old('loan_start_date') }}" min="{{ now()->toDateTimeLocalString('minute') }}">
                ...
                <div class="card shadow-sm mb-4">
                    <div class="card-header bg-light py-3">
                        <h2 class="h5 card-title mb-0 fw-semibold">{{ __('BAHAGIAN 2 | MAKLUMAT PEGAWAI BERTANGGUNGJAWAB') }}</h2>
                    </div>
                    <div class="card-body p-4">
                        <div class="form-check mb-3">
                            <input type="checkbox" name="applicant_is_responsible_officer_checkbox" id="applicant_is_responsible_officer_checkbox" value="1" class="form-check-input" {{ old('applicant_is_responsible_officer_checkbox', true) ? 'checked' : '' }}>
                        ...
                        <div id="responsible-officer-fields" class="{{ old('applicant_is_responsible_officer_checkbox', true) ? 'd-none' : '' }}">
                            <p class="form-text small mb-3 text-muted">{{ __('Bahagian ini hanya perlu diisi jika Pegawai Bertanggungjawab bukan Pemohon.') }}</p>
                            <div class="mb-3">
                                    ...
                                    @if(!empty($responsibleOfficers) && $responsibleOfficers->count())
                                        @foreach ($responsibleOfficers as $officer)
                                            ...
                                            <option value="{{ $officer->id }}" {{ old('responsible_officer_id') == $officer->id ? 'selected' : '' }}>
                                                {{ e($officer->name) }} ({{ e(optional($officer->position)->name) ?? __('Posisi T/D') }} - {{ e(optional($officer->grade)->name) ?? __('Gred T/D') }})
                                            </option>
                                            @endif
                                        @endforeach
                                    @endif
                ...
                <div class="card shadow-sm mb-4">
                    <div class="card-header bg-light py-3">
                        <h2 class="card-title h5 mb-0 fw-semibold">{{ __('MAKLUMAT PEGAWAI PENYOKONG') }}<span class="text-danger">*</span></h2>
                    </div>
                    <div class="card-body p-4">
                         <div class="mb-3">
                            ...
                            <div class="form-text small text-muted">{{__('Pegawai Penyokong mestilah sekurang-kurangnya Gred :grade atau setara.', ['grade' => $minSupportGradeLevel])}}</div>
                ...
                <div class="card shadow-sm mb-4">
                    <div class="card-header bg-light py-3 d-flex justify-content-between align-items-center">
                        <h2 class="card-title h5 mb-0 fw-semibold">{{ __('BAHAGIAN 3 | MAKLUMAT PERALATAN DIMOHON') }}</h2>
                        <button type="button" id="add-item-button" class="btn btn-sm btn-outline-primary d-inline-flex align-items-center">
                            <i class="bi bi-plus-circle me-1"></i> {{ __('Tambah Item') }}
                        </button>
                    </div>
                    <div class="card-body p-4">
                        <p class="form-text small mb-3 text-muted">{{ __('Sila senaraikan peralatan ICT yang diperlukan.') }}</p>
                        <div id="equipment-items-container" class="vstack gap-3">
                            ...
                            <div class="row g-3 align-items-end border-bottom pb-3 item-row" id="item-row-{{ $index }}">
                                <div class="col-md-4">
                                    ...
                                    <select name="items[{{ $index }}][equipment_type]" id="items_{{ $index }}_equipment_type" class="form-select form-select-sm @error('items.'.$index.'.equipment_type') is-invalid @enderror" required>
                                        ...
                                        @foreach($equipmentAssetTypeOptions ?? [] as $typeKey => $typeLabel)
                                            <option value="{{ $typeKey }}" {{ (isset($item['equipment_type']) && $item['equipment_type'] == $typeKey) ? 'selected' : '' }}>{{ e(__($typeLabel)) }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-2 d-flex align-items-end">
                                    <button type="button" class="btn btn-sm btn-outline-danger w-100 remove-item-button">
                                       <i class="bi bi-trash3-fill"></i> <span class="d-none d-sm-inline">{{ __('Buang') }}</span>
                                    </button>
                                </div>
                            </div>
                            @endforeach
                        ...
                        @error('items') <div class="d-block text-danger small mt-2">{{ $message }}</div> @enderror
                         <div class="form-text small mt-1 text-muted">{{__('Pastikan sekurang-kurangnya satu item peralatan disenaraikan.')}}</div>
                    </div>
                </div>
                ...
                <div class="text-center mt-4 pt-3">
                    <a href="{{ route('loan-applications.index') }}" class="btn btn-outline-secondary btn-lg px-4 me-lg-3 me-2 mb-2 mb-lg-0 d-inline-flex align-items-center">
...
{{-- Hidden template for new equipment items --}}
<template id="equipment-item-template">
    <div class="row g-3 align-items-end border-bottom pb-3 item-row">
        <div class="col-md-4">
            <label for="items_INDEX_equipment_type" class="form-label fw-semibold small">{{ __('Jenis Peralatan') }}<span class="text-danger">*</span></label>
            <select name="items[INDEX][equipment_type]" data-name-template="items[INDEX][equipment_type]" class="form-select form-select-sm" required>
                <option value="">- {{ __('Pilih Jenis') }} -</option>
                @foreach($equipmentAssetTypeOptions ?? [] as $typeKey => $typeLabel)
                    <option value="{{ $typeKey }}">{{ e(__($typeLabel)) }}</option>
                @endforeach
            </select>
            <div class="invalid-feedback d-block" data-error-for="items.INDEX.equipment_type"></div>
        </div>
        <div class="col-md-2">
            <label for="items_INDEX_quantity_requested" class="form-label fw-semibold small">{{ __('Kuantiti') }}<span class="text-danger">*</span></label>
            <input type="number" name="items[INDEX][quantity_requested]" data-name-template="items[INDEX][quantity_requested]" class="form-control form-control-sm" min="1" value="1" required>
            <div class="invalid-feedback d-block" data-error-for="items.INDEX.quantity_requested"></div>
        </div>
        <div class="col-md-4">
            <label for="items_INDEX_notes" class="form-label fw-semibold small">{{ __('Catatan Tambahan') }}</label>
            <input type="text" name="items[INDEX][notes]" data-name-template="items[INDEX][notes]" class="form-control form-control-sm" placeholder="Cth: Model spesifik, perisian khas">
            <div class="invalid-feedback d-block" data-error-for="items.INDEX.notes"></div>
        </div>
        <div class="col-md-2 d-flex align-items-end">
            <button type="button" class="btn btn-sm btn-outline-danger w-100 remove-item-button">
                <i class="bi bi-trash3-fill"></i> <span class="d-none d-sm-inline">{{ __('Buang') }}</span>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        let itemIndex = container.querySelectorAll('.item-row').length > 0 ? container.querySelectorAll('.item-row').length : {{ count(old('items', [['dummy']])) }};
        ...
        function updateRemoveButtonsVisibility() {
            ...
            rows.forEach(row => {
                ...
                if (removeButton) {
                    removeButton.style.display = (rows.length <= 1) ? 'none' : 'flex'; // 'flex' to match d-inline-flex
                }
            });
        }
        ...
        if (addButton && templateNode && container) {
            addButton.addEventListener('click', function() {
                ...
                newRowElement.querySelectorAll('[data-name-template]').forEach(el => {
                    ...
                    const newId = nameTemplate.replace(/\[INDEX\]/g, `_${itemIndex}_`).replace(/[\[\]]/g, ''); // e.g., items_0_equipment_type
                    el.id = newId;
                    const label = newRowElement.querySelector(`label[for="${nameTemplate.replace('INDEX', 'INDEX')}"]`); // Placeholder for matching, might need refinement
                    if(label && label.getAttribute('for') === `items_INDEX_${el.name.split('[')[2].replace(']', '')}`) {
        ...
        function toggleResponsibleOfficerDisplay() {
            if (!responsibleOfficerCheckbox || !responsibleOfficerSection || !responsibleOfficerIdField || !responsibleOfficerAsterisk) return;
