        ...
        <div class="col-lg-10 col-xl-9">
            ...
            @if ($loanApplication->status === \App\Models\LoanApplication::STATUS_REJECTED && $loanApplication->rejection_reason)
                <div class="alert alert-danger bg-danger-subtle border-danger-subtle p-3 mb-4 shadow-sm rounded-3">
                    <h5 class="alert-heading h6 text-danger-emphasis fw-semibold"><i class="bi bi-x-octagon-fill me-2"></i>{{ __('Sebab Penolakan') }}:</h5>
                    <p class="mb-0 text-danger-emphasis small" style="white-space: pre-wrap;">{{ e($loanApplication->rejection_reason) }}</p>
                </div>
            @endif
            ...
            <div class="card shadow-lg rounded-3 border-0">
                <div class="card-body p-sm-4 p-3 vstack gap-4">
                    ...
                    <section aria-labelledby="loan-details">
                        <h2 id="loan-details" class="h5 fw-semibold text-dark mb-3 mt-4 border-bottom pb-2">{{ __('BUTIRAN PERMOHONAN PINJAMAN') }}</h2>
                        <dl class="row g-3 small">
                            <dt class="col-sm-12 fw-medium text-muted">{{ __('Tujuan Permohonan:') }}</dt>
                            <dd class="col-sm-12 text-dark bg-light p-2 rounded-2 border" style="white-space: pre-wrap;">{{ e($loanApplication->purpose ?? 'N/A') }}</dd>

                            <dt class="col-md-4 col-lg-3 fw-medium text-muted">{{ __('Lokasi Penggunaan Peralatan:') }}</dt>
                            ...
                            <dd class="col-md-8 col-lg-9 text-dark">{{ e($loanApplication->return_location ?? (e($loanApplication->location) ?: 'N/A')) }}</dd>

                            <dt class="col-md-4 col-lg-3 fw-medium text-muted">{{ __('Tarikh & Masa Pinjaman:') }}</dt>
                            <dd class="col-md-8 col-lg-9 text-dark">{{ optional($loanApplication->loan_start_date)->translatedFormat('d M Y, h:i A') ?? __('N/A') }}</dd>

                            <dt class="col-md-4 col-lg-3 fw-medium text-muted">{{ __('Tarikh & Masa Dijangka Pulang:') }}</dt>
                    ...
                    <section aria-labelledby="responsible-officer-information">
                        <h2 id="responsible-officer-information" class="h5 fw-semibold text-dark mb-3 mt-4 border-bottom pb-2">{{ __('BAHAGIAN 2 | MAKLUMAT PEGAWAI BERTANGGUNGJAWAB') }}</h2>
                        ...
                        @elseif(optional($loanApplication->responsibleOfficer)->id === $loanApplication->user_id || !$loanApplication->responsibleOfficer)
                            <p class="text-muted small"><i class="bi bi-info-circle me-1"></i>{{ __('Pemohon adalah Pegawai Bertanggungjawab.') }}</p>
                        @else
                             <p class="text-muted small"><i class="bi bi-info-circle me-1"></i>{{ __('Maklumat Pegawai Bertanggungjawab tidak dinyatakan.') }}</p>
                        @endif
                    </section>

                    {{-- Supporting Officer --}}
                    <section aria-labelledby="supporting-officer-info">
                        <h2 id="supporting-officer-info" class="h5 fw-semibold text-dark mb-3 mt-4 border-bottom pb-2">{{ __('MAKLUMAT PEGAWAI PENYOKONG') }}</h2>
                        @if($loanApplication->supportingOfficer)
                            <x-user-info-card :user="$loanApplication->supportingOfficer" title=""/>
                        @else
                            <p class="text-muted small"><i class="bi bi-info-circle me-1"></i>{{ __('Tiada Pegawai Penyokong ditetapkan untuk permohonan ini.') }}</p>
                        @endif
                    </section>

                    {{-- Section 3: Requested Equipment --}}
                    <section aria-labelledby="equipment-requested">
                        <h2 id="equipment-requested" class="h5 fw-semibold text-dark mb-3 mt-4 border-bottom pb-2">{{ __('BAHAGIAN 3 | MAKLUMAT PERALATAN DIMOHON') }}</h2>
                                    ...
                                    <tbody>
                                        @foreach ($loanApplication->applicationItems as $item)
                                            <tr>
                                                ...
                                                <td class="small text-dark">{{ e(\App\Models\Equipment::getAssetTypeOptions()[$item->equipment_type] ?? Str::title(str_replace('_', ' ', $item->equipment_type))) }}</td>
                    ...
                    <section aria-labelledby="applicant-confirmation">
                        <h2 id="applicant-confirmation" class="h5 fw-semibold text-dark mb-3 mt-4 border-bottom pb-2">{{ __('BAHAGIAN 4 | PENGESAHAN PEMOHON') }}</h2>
                        <div class="small">
                            @if ($loanApplication->applicant_confirmation_timestamp)
                                <span class="fw-semibold text-success"><i class="bi bi-check-circle-fill me-1"></i>{{ __('Telah Disahkan oleh Pemohon') }}</span>
                                <span class="text-dark"> {{ __('pada') }} {{ optional($loanApplication->applicant_confirmation_timestamp)->translatedFormat('d M Y, h:i A') }}</span>
                            @else
                                <span class="fw-semibold text-warning"><i class="bi bi-hourglass-split me-1"></i>{{ __('Belum Disahkan oleh Pemohon') }}</span>
                            @endif
                        </div>
                    </section>
                    ...
                    <section aria-labelledby="approval-history">
                        <h2 id="approval-history" class="h5 fw-semibold text-dark mb-3 mt-4 border-bottom pb-2">{{ __('SEJARAH KELULUSAN & TINDAKAN') }}</h2>
                                    ...
                                    <tbody>
                                        @foreach ($loanApplication->approvals->sortBy('created_at') as $approval)
                                            <tr>
                                                ...
                                                <td class="small text-muted">{{ optional($approval->approval_timestamp ?? $approval->updated_at)->translatedFormat('d M Y, h:i A') ?? __('N/A') }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                        ...
                        @else
                            <p class="text-muted small fst-italic"><i class="bi bi-info-circle me-1"></i>{{ __('Tiada sejarah kelulusan untuk permohonan ini.') }}</p>
                        @endif
                    </section>

                    {{-- Loan Transaction History --}}
                    <section aria-labelledby="transaction-history">
                        <h2 id="transaction-history" class="h5 fw-semibold text-dark mb-3 mt-4 border-bottom pb-2">{{ __('SEJARAH TRANSAKSI PINJAMAN') }}</h2>
                                ...
                                <div class="card shadow-sm border">
                                    <div class="card-body p-3">
                                        ...
                                        @if($transaction->loanTransactionItems->count() > 0)
                                            <h4 class="small fw-bold text-uppercase text-muted mb-1 mt-3">{{ __('Item Terlibat:') }}</h4>
                                        ...
                                        @if($transaction->issue_notes && $transaction->isIssue())
                                         <p class="mt-2 small text-muted mb-1"><strong class="fw-medium">{{ __('Nota Pengeluaran:') }}</strong> <span style="white-space: pre-wrap;">{{ e($transaction->issue_notes) }}</span></p>
                                        @endif
                                        @if($transaction->return_notes && $transaction->isReturn())
                                         <p class="mt-2 small text-muted mb-1"><strong class="fw-medium">{{ __('Nota Pemulangan:') }}</strong> <span style="white-space: pre-wrap;">{{ e($transaction->return_notes) }}</span></p>
                                        @endif
                        ...
                        @else
                            <p class="text-muted small fst-italic"><i class="bi bi-info-circle me-1"></i>{{ __('Tiada sejarah transaksi untuk permohonan ini.') }}</p>
                        @endif
                    </section>
                             ...
                             @if ($loanApplication->isDraft() || $loanApplication->isRejected())
                                <form action="{{ route('loan-applications.submit', $loanApplication) }}" method="POST" class="d-inline" onsubmit="return confirm('{{__('Adakah anda pasti untuk menghantar permohonan ini untuk kelulusan?')}}');">
                                    ...
                                    <button type="submit" class="btn btn-success d-inline-flex align-items-center">
                                        <i class="bi bi-send-check-fill me-1"></i> {{ $loanApplication->isRejected() ? __('Hantar Semula Permohonan') : __('Hantar untuk Kelulusan') }}
                                    </button>
                                </form>
                            @endif
                        @endcan
