{{-- resources/views/admin/grades/grade-index.blade.php --}}
@extends('layouts.app')
    ...
    <div class="container py-4">
        ...
        <div class="card shadow-sm">
            <div class="card-header bg-light d-flex flex-wrap justify-content-between align-items-center gap-2 py-3">
                ...
                {{-- Place for search/filter form if needed --}}
            </div>
            @if ($grades->isEmpty())
                <div class="card-body">
                    ...
                    <table class="table table-hover table-striped mb-0 align-middle">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th class="small text-uppercase text-muted fw-medium px-3 py-2">{{ __('Tahap (Angka)') }}</th>
                                <th class="small text-uppercase text-muted fw-medium px-3 py-2" style="min-width: 250px;">{{ __('Keterangan') }}</th>
                                <th class="small text-uppercase text-muted fw-medium text-center px-3 py-2">{{ __('Gred Pelulus?') }}</th>
                                <th class="small text-uppercase text-muted fw-medium px-3 py-2">{{ __('Diluluskan Oleh Gred Min.') }}</th>
                                ...
                                <tr>
                                    ...
                                    <td class="px-3 py-2 small text-center">
                                        @if ($grade->is_approver_grade)
                                            <span class="badge rounded-pill text-bg-success">{{ __('Ya') }}</span>
                                        @else
                                            <span class="badge rounded-pill text-bg-secondary">{{ __('Tidak') }}</span>
                                        @endif
                                    </td>
