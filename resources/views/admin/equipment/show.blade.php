            ...
            <div class="col-lg-9 col-xl-8">
                <div
                    ...
                    <h1 class="h2 fw-bold text-dark mb-2 mb-sm-0 d-flex align-items-center">
                        <i class="bi bi-pc-display-horizontal me-2"></i>{{ __('Butiran Peralatan') }}: <span
                    ...
                    <div>
                        ...
                        {{-- ADJUSTMENT: Verified @can directive is correct --}}
                ...
                <div class="card shadow-sm mb-4 motac-card">
                    <div class="card-header bg-light py-3 motac-card-header">
                        <h2 class="h5 card-title fw-semibold mb-0 d-flex align-items-center">
                            <i class="bi bi-info-circle-fill me-2 text-primary"></i> {{ __('Maklumat Am') }}
                        </h2>
                    </div>
                    <div class="card-body p-3 p-md-4">
                        {{-- The <dl> list remains as it was well-structured --}}
                        <dl class="row g-3 small">
                            ...
                            <dd class="col-sm-8 col-lg-9"><x-equipment-status-badge :status="$equipment->status" /></dd>
                            ...
                            <dd class="col-sm-8 col-lg-9 text-dark font-monospace">{{ $equipment->serial_number ?? 'N/A' }}
                            </dd>
                            <dt class="col-sm-4 col-lg-3 fw-medium text-muted">{{ __('Kod Item') }}:</dt>
                            <dd class="col-sm-8 col-lg-9 text-dark font-monospace">{{ $equipment->item_code ?? 'N/A' }}
                            </dd>
                            ...
                            <dd class="col-sm-8 col-lg-9 text-dark">
                                {{ $equipment->purchase_date ? $equipment->purchase_date->translatedFormat('d M Y') : 'N/A' }}
                            </dd>
                ...
                {{-- Loan History and other cards follow the same pattern --}}
                @if ($equipment->activeLoanTransactionItem)
                    <div class="card shadow-sm mb-4 motac-card">
                        <div class="card-header bg-light py-3 motac-card-header">
                            <h2 class="h5 card-title fw-semibold mb-0 d-flex align-items-center">
                                <i
                                    class="bi bi-person-check-fill me-2 text-primary"></i>{{ __('Maklumat Pinjaman Aktif') }}
                            </h2>
                        </div>
                        <div class="card-body p-3 p-md-4">
                            <dl class="row g-2 small">
                                ...
                                <dd class="col-sm-8 text-dark">
                                    @if ($equipment->activeLoanTransactionItem->loanTransaction?->loanApplication?->user)
                                        <a href="{{ route('settings.users.show', $equipment->activeLoanTransactionItem->loanTransaction->loanApplication->user) }}"
                                ...
                                <dd class="col-sm-8 text-dark">
                                    @if ($equipment->activeLoanTransactionItem->loanTransaction?->loanApplication)
                                        <a href="{{ route('loan-applications.show', $equipment->activeLoanTransactionItem->loanTransaction->loanApplication) }}"
                                            class="text-decoration-none">
                                ...
                                <dd
                                    class="col-sm-8 text-dark {{ optional($equipment->activeLoanTransactionItem->loanTransaction->loanApplication)->isOverdue() ? 'text-danger fw-bold' : '' }}">
                ...
                <div class="card shadow-sm motac-card">
                    <div class="card-header bg-light py-3 motac-card-header">
                        <h2 class="h5 card-title fw-semibold mb-0 d-flex align-items-center">
                            <i
                                class="bi bi-list-ol me-2 text-primary"></i>{{ __('Sejarah Transaksi Pinjaman Peralatan Ini') }}
                        </h2>
                    </div>
                                    ...
                                    <tbody>
                                        @foreach ($equipment->loanTransactionItems->sortByDesc('loanTransaction.transaction_date') as $transactionItem)
                                            <tr>
                                                ...
                                                <td class="small px-3 py-2 text-muted">
                                                    {{ optional(optional($transactionItem->loanTransaction->loanApplication)->user)->name ?? __('N/A') }}
                                                </td>
                                                <td class="small px-3 py-2">
                                                    <span
                                                        class="badge rounded-pill {{ $transactionItem->loanTransaction->type === \App\Models\LoanTransaction::TYPE_ISSUE ? 'bg-info-subtle text-info-emphasis' : 'bg-primary-subtle text-primary-emphasis' }} fw-normal">
                        ...
                        @else
                            <div class="p-3">
                                <p class="small text-muted mb-0"><i
                                        class="bi bi-info-circle me-1"></i>{{ __('Tiada sejarah pinjaman untuk peralatan ini.') }}
                                </p>
                            </div>
                        @endif
                    </div>
                    {{-- ADJUSTMENT: Verified @can directive is correct for the delete action --}}
                    @can('delete', $equipment)
                        <div class="card-footer bg-light text-end py-3 border-top">
                            <form action="{{ route('resource-management.equipment-admin.destroy', $equipment) }}"
                                method="POST"
