            ...
            <div class="col-lg-9 col-xl-8">
                <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 pb-2 border-bottom">
                    <h1 class="h2 fw-bold text-dark mb-0 d-flex align-items-center">
                        <i class="bi bi-pencil-square me-2"></i>{{ __('Kemaskini Peralatan') }} <span
                ...
                <form action="{{ route('resource-management.equipment-admin.update', $equipment) }}" method="POST"
                    ...
                    <div class="card shadow-sm motac-card">
                        ...
                        <div class="card-body p-3 p-md-4 motac-card-body">
                            <div class="row g-3">
                                {{-- Form fields are populated with $equipment data --}}
                                <div class="col-md-6">
                                    ...
                                    <select name="asset_type" id="asset_type" required
                                        ...
                                        @foreach ($assetTypes as $typeValue => $typeLabel)
                                            <option value="{{ $typeValue }}"
                                                {{ old('asset_type', $equipment->asset_type) == $typeValue ? 'selected' : '' }}>
                                                {{ __($typeLabel) }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                {{-- Continue for all other fields: brand, model, serial_number, etc. --}}
                                <div class="col-md-6">
                                ...
                                <div class="col-md-6">
                                    <label for="item_code"
                                        class="form-label fw-medium">{{ __('Kod Item (KEW.PA-2/KEW.PA-3)') }}</label>
                                    <input type="text" name="item_code" id="item_code"
                                        class="form-control form-control-sm @error('item_code') is-invalid @enderror"
                                        value="{{ old('item_code', $equipment->item_code) }}">
                                    @error('item_code')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-md-6">
                                    <label for="purchase_price"
                                        class="form-label fw-medium">{{ __('Harga Pembelian (RM)') }}</label>
                                    ...
                                    <select name="status" id="status" required
                                        ...
                                        @foreach ($statusOptions as $statusValue => $statusLabel)
                                            <option value="{{ $statusValue }}"
                                                {{ old('status', $equipment->status) == $statusValue ? 'selected' : '' }}>
                                                {{ __($statusLabel) }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="condition_status" id="condition_status" required
                                        ...
                                        @foreach ($conditionStatusOptions as $statusValue => $statusLabel)
                                            <option value="{{ $statusValue }}"
                                                {{ old('condition_status', $equipment->condition_status) == $statusValue ? 'selected' : '' }}>
                                                {{ __($statusLabel) }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select name="department_id" id="department_id"
                                        ...
                                        @isset($departments)
                                            @foreach ($departments as $id => $name)
                                                <option value="{{ $id }}"
                                                    {{ old('department_id', $equipment->department_id) == $id ? 'selected' : '' }}>
                                                    {{ $name }}
                                                </option>
                                            @endforeach
                                        @endisset
                                ...
                                <div class="col-md-6">
                                    <label for="location_id"
                                        class="form-label fw-medium">{{ __('Lokasi Tersimpan (Jika ada)') }}</label>
                                    <select name="location_id" id="location_id"
                                        class="form-select form-select-sm @error('location_id') is-invalid @enderror">
                                        <option value="">-- {{ __('Pilih Lokasi Tersimpan') }} --</option>
                                        @isset($locations)
                                            @foreach ($locations as $id => $name)
                                                <option value="{{ $id }}"
                                                    {{ old('location_id', $equipment->location_id) == $id ? 'selected' : '' }}>
                                                    {{ $name }}
                                                </option>
                                            @endforeach
                                        @endisset
                                ...
                                <div class="col-md-6">
                                    <label for="equipment_category_id"
                                        class="form-label fw-medium">{{ __('Kategori Peralatan') }}</label>
                                    <select name="equipment_category_id" id="equipment_category_id"
                                        class="form-select form-select-sm @error('equipment_category_id') is-invalid @enderror">
                                        <option value="">-- {{ __('Pilih Kategori') }} --</option>
                                        @isset($equipmentCategories)
                                            @foreach ($equipmentCategories as $id => $name)
                                                <option value="{{ $id }}"
                                                    {{ old('equipment_category_id', $equipment->equipment_category_id) == $id ? 'selected' : '' }}>
                                                    {{ $name }}
                                                </option>
                                            @endforeach
                                        @endisset
                                ...
                                <div class="col-md-6">
                                    <label for="sub_category_id"
                                        class="form-label fw-medium">{{ __('Sub-Kategori Peralatan') }}</label>
                                    <select name="sub_category_id" id="sub_category_id"
                                        class="form-select form-select-sm @error('sub_category_id') is-invalid @enderror">
                                        <option value="">-- {{ __('Pilih Sub-Kategori') }} --</option>
                                        @isset($subCategories)
                                            @foreach ($subCategories as $id => $name)
                                                <option value="{{ $id }}"
                                                    {{ old('sub_category_id', $equipment->sub_category_id) == $id ? 'selected' : '' }}>
                                                    {{ $name }}
                                                </option>
                                            @endforeach
                                        @endisset
                                ...
                                <div class="col-md-6">
                                    <label for="acquisition_type"
                                        class="form-label fw-medium">{{ __('Jenis Perolehan') }}</label>
                                    <select name="acquisition_type" id="acquisition_type"
                                        class="form-select form-select-sm @error('acquisition_type') is-invalid @enderror">
                                        <option value="">-- {{ __('Pilih Jenis Perolehan') }} --</option>
                                        @foreach ($acquisitionTypes as $typeValue => $typeLabel)
                                            <option value="{{ $typeValue }}"
                                                {{ old('acquisition_type', $equipment->acquisition_type) == $typeValue ? 'selected' : '' }}>
                                                {{ __($typeLabel) }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6">
                                    <label for="classification"
                                        class="form-label fw-medium">{{ __('Klasifikasi') }}</label>
                                    <select name="classification" id="classification"
                                        class="form-select form-select-sm @error('classification') is-invalid @enderror">
                                        <option value="">-- {{ __('Pilih Klasifikasi') }} --</option>
                                        @foreach ($classifications as $typeValue => $typeLabel)
                                            <option value="{{ $typeValue }}"
                                                {{ old('classification', $equipment->classification) == $typeValue ? 'selected' : '' }}>
                                                {{ __($typeLabel) }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="funded_by" id="funded_by"
                                        value="{{ old('funded_by', $equipment->funded_by) }}"
                                        class="form-control form-control-sm @error('funded_by') is-invalid @enderror"
                                        placeholder="{{ __('Cth: Geran Persekutuan, Peruntukan Negeri') }}">
                                    @error('funded_by')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-12">
                                    <label for="supplier_name"
                                        class="form-label fw-medium">{{ __('Nama Pembekal') }}</label>
                                    <input type="text" name="supplier_name" id="supplier_name"
                                        ...
                                        placeholder="{{ __('Nama penuh pembekal peralatan') }}">
                                    @error('supplier_name')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-12">
                                    ...
                                    <textarea name="description" id="description" rows="3"
                                        ...
                                        placeholder="{{ __('Cth: Spesifikasi ringkas, warna, atau sebarang maklumat berkaitan') }}">{{ old('description', $equipment->description) }}</textarea>
                                    @error('description')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-12">
                                    ...
                                    <textarea name="notes" id="notes" rows="3"
                                        ...
                                        placeholder="{{ __('Sebarang nota atau catatan lain') }}">{{ old('notes', $equipment->notes) }}</textarea>
                                    @error('notes')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
