...
<div class="container-fluid py-4"> {{-- Using container-fluid for potentially wider table display --}}
    ...
    <div class="card shadow-sm">
         <div class="card-header bg-light d-flex flex-wrap justify-content-between align-items-center gap-2 py-3">
            ...
            {{-- Consider adding a search/filter form here if not part of a Livewire component --}}
            {{-- Example:
            <form action="{{ route('admin.users.index') }}" method="GET" class="d-flex gap-2">
                <input type="text" name="search" class="form-control form-control-sm" placeholder="{{ __('Cari Nama, Emel, Peranan...') }}" value="{{ request('search') }}">
                ...
                <table class="table table-hover table-striped mb-0 align-middle">
                    <thead class="table-light">
                        <tr>
                            ...
                            {{-- <th class="small text-uppercase text-muted fw-medium px-3 py-2">{{ __('ID Pekerja') }}</th> --}}
                    ...
                    <tbody>
                        @foreach ($users as $user)
                            <tr>
                                ...
                                {{-- <td class="px-3 py-2 small text-muted font-monospace">{{ $user->employee_id ?? 'N/A' }}</td> --}}
