...
<div class="container py-4"> {{-- Changed to container for consistent width, use container-fluid if full width needed --}}
    <div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4 pb-2 border-bottom">
        ...
        <div>
            @can('update', $equipment) {{-- Policy check from Admin context --}}
                <a href="{{ route('resource-management.equipment-admin.edit', $equipment) }}" class="btn btn-outline-primary btn-sm me-2 d-inline-flex align-items-center">
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body p-4">
                    <dl class="row mb-0">
                        ...
                        <dd class="col-sm-7 col-md-8 small">{{ e($equipment->asset_type_label ?? $equipment->asset_type ?? __('N/A')) }}</dd>
                        ...
                        <dd class="col-sm-7 col-md-8 small font-monospace">{{ e($equipment->tag_id ?? __('N/A')) }}</dd>

                        <dt class="col-sm-5 col-md-4 text-muted small">{{ __('Nombor Siri') }}</dt>
                        <dd class="col-sm-7 col-md-8 small font-monospace">{{ e($equipment->serial_number ?? __('N/A')) }}</dd>

                        <dt class="col-sm-5 col-md-4 text-muted small">{{ __('Kod Item') }}</dt>
                        <dd class="col-sm-7 col-md-8 small font-monospace">{{ e($equipment->item_code ?? __('N/A')) }}</dd>

                        <dt class="col-sm-5 col-md-4 text-muted small">{{ __('Tarikh Pembelian') }}</dt>
                        <dd class="col-sm-7 col-md-8 small">{{ $equipment->purchase_date?->translatedFormat('d M Y') ?? __('N/A') }}</dd>

                        <dt class="col-sm-5 col-md-4 text-muted small">{{ __('Tarikh Tamat Waranti') }}</dt>
                        <dd class="col-sm-7 col-md-8 small">{{ $equipment->warranty_expiry_date?->translatedFormat('d M Y') ?? __('N/A') }}</dd>
                        ...
                        <dd class="col-sm-7 col-md-8 small mt-2" style="white-space: pre-wrap;">{{ e($equipment->notes) }}</dd>
                        @endif
                    </dl>
                </div>
            </div>
             ...
             <div class="card shadow-sm">
                ...
                <div class="card-body p-4">
                    <dl class="row mb-0">
                        ...
                        <dd class="col-sm-7 small">{{ $equipment->created_at?->translatedFormat('d M Y, H:i A') ?? __('N/A') }}</dd>
                        ...
                        <dd class="col-sm-7 small">{{ $equipment->updated_at?->translatedFormat('d M Y, H:i A') ?? __('N/A') }}</dd>
                    </dl>
                </div>
            </div>
        </div>
                ...
                <div class="table-responsive">
                    <table class="table table-sm table-hover table-striped mb-0 align-middle">
                        <thead class="table-light">
                            <tr>
                                <th class="py-2 px-3 small text-uppercase text-muted fw-medium">{{ __('ID Transaksi') }}</th>
                                <th class="py-2 px-3 small text-uppercase text-muted fw-medium">{{ __('Permohonan #') }}</th>
                                <th class="py-2 px-3 small text-uppercase text-muted fw-medium">{{ __('Peminjam') }}</th>
                                <th class="py-2 px-3 small text-uppercase text-muted fw-medium">{{ __('Jenis Transaksi') }}</th>
                                <th class="py-2 px-3 small text-uppercase text-muted fw-medium">{{ __('Status Item Dalam Transaksi Ini') }}</th>
                                <th class="py-2 px-3 small text-uppercase text-muted fw-medium">{{ __('Tarikh Transaksi') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($equipment->loanTransactionItems->sortByDesc('loanTransaction.transaction_date') as $transactionItem)
                                <tr>
                                    ...
                                    <td class="py-2 px-3 small">
                                     ...
                                     <td class="py-2 px-3 small text-muted">{{ optional(optional($transactionItem->loanTransaction->loanApplication)->user)->name ?? __('N/A')}}</td>
                                    <td class="py-2 px-3 small">
                                        <span class="badge rounded-pill {{ $transactionItem->loanTransaction->type === \App\Models\LoanTransaction::TYPE_ISSUE ? 'bg-info text-dark' : 'bg-primary' }}">
                                            {{ __(Str::ucfirst($transactionItem->loanTransaction->type_label)) }} {{-- Assuming type_label accessor --}}
                                        </span>
                                    </td>
                                    <td class="py-2 px-3 small">
                                        <x-equipment-status-badge :status="$transactionItem->status" :type="'transaksi_item'" />
                                    </td>
                                    <td class="py-2 px-3 small">{{ $transactionItem->loanTransaction->transaction_date?->translatedFormat('d M Y, h:i A') }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
