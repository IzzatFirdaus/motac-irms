...
<div class="container-fluid py-4">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        @if (Route::has('approvals.history'))
            <a href="{{ route('approvals.history', ['status' => 'all']) }}" class="motac-btn-outline btn-sm d-inline-flex align-items-center"><i class="bi bi-clock-history me-1"></i>{{ __('dashboard.view_my_approval_history') }}</a>
        @endif
    </div>
    ...
    <div class="row">
        <div class="col-12">
            <div class="motac-card shadow-sm mb-4">
                <div class="motac-card-header py-3 d-flex align-items-center"><i class="bi bi-card-checklist me-2 text-primary"></i><h6 class="m-0 fw-bold text-primary">{{ __('dashboard.apps_awaiting_your_action') }}</h6></div>
    ...
    <div class="row mt-2">
        <div class="col-lg-6 mb-4">
            <div class="motac-card shadow-sm h-100">
                <div class="motac-card-header py-3 d-flex align-items-center"><i class="bi bi-graph-up me-2 text-primary"></i><h6 class="m-0 fw-bold text-primary">{{ __('dashboard.personal_approval_stats') }}</h6></div>
        ...
        <div class="col-lg-6 mb-4">
            <div class="motac-card shadow-sm h-100">
                <div class="motac-card-header py-3 d-flex align-items-center"><i class="bi bi-info-circle-fill me-2 text-primary"></i><h6 class="m-0 fw-bold text-primary">{{ __('dashboard.approval_guidance_title') }}</h6></div>
                <div class="motac-card-body">
                    <p class="small text-muted mb-3">{{ __('dashboard.approval_guidance_text') }}</p>
                    @if (config('system_links.approval_guidelines_url'))
                        <a href="{{ config('system_links.approval_guidelines_url') }}" target="_blank" rel="noopener noreferrer" class="motac-btn-outline btn-sm d-inline-flex align-items-center"><i class="bi bi-book-half me-1"></i>{{ __('dashboard.read_full_guidelines') }}</a>
                    @endif
                </div>
            </div>
        </div>
