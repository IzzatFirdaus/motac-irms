...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $statuses = [
            'draft', 'pending_support', 'pending_hod_review', 'pending_bpm_review',
            'approved', 'rejected', 'partially_issued', 'issued',
        ...
        Schema::table('loan_applications', function (Blueprint $table) {
            ...
            foreach($foreignKeys as $key) {
                if (Schema::hasColumn('loan_applications', $key)) {
                     try { $table->dropForeign([$key]); } catch (\Exception $e) { Log::warning("Failed to drop FK {$key} on loan_applications: ". $e->getMessage());}
                }
            }
        });
