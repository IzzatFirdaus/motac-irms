...
use App\Models\User; // Import User model to access constants
...
return new class extends Migration
{
  public function up(): void
  {
    Schema::table('users', function (Blueprint $table) {
      ...
      // **CRITICAL FIX**: Ensure ENUM definition includes ALL valid keys from User model constants
      $serviceStatusEnumValues = [
          ...
          // Add other User::SERVICE_STATUS_* constants here if you define more
      ];
      // Remove duplicates and nulls just in case constants overlap or are undefined
      ...
      if(empty($serviceStatusEnumValues)) {
          ...
          $serviceStatusEnumValues = ['1', '2', '3', '4']; // Ensure this fallback matches your actual needs
      }
      ...
      $statusEnumValues = [User::STATUS_ACTIVE, User::STATUS_INACTIVE];
      $statusEnumValues = array_unique(array_filter($statusEnumValues, fn($value) => !is_null($value)));
       if(empty($statusEnumValues)) {
          Log::error("Status enum values could not be determined from User model constants. Using hardcoded fallback in migration. PLEASE CHECK User.php CONSTANTS.");
          $statusEnumValues = ['active', 'inactive'];
      }
      $defaultUserStatus = defined(User::class.'::STATUS_ACTIVE') ? User::STATUS_ACTIVE : 'active';
      ...
      if (Schema::hasTable('employees')) {
        ...
        $table->unsignedBigInteger('employee_id')->nullable()->after('profile_photo_path')->comment('Link to employees table if it exists');
      }
  ...
  {
    Schema::table('users', function (Blueprint $table) {
      $foreignKeysToDrop = ['department_id', 'position_id', 'grade_id', 'created_by', 'updated_by', 'deleted_by'];
      if (Schema::hasTable('employees') && Schema::hasColumn('users', 'employee_id')) {
          try {
